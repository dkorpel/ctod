 ...
 */
enum TREE_SITTER_LANGUAGE_VERSION = 11;

/**
 * The earliest ABI version that is supported by the current version of the
 * library.
 */
enum TREE_SITTER_MIN_COMPATIBLE_LANGUAGE_VERSION = 9;
 ...
 */
bool ts_parser_set_included_ranges(TSParser* self, const(TSRange)* ranges, uint length);
 ...
 * `length` pointer.
 */
const(TSRange)* ts_parser_included_ranges(const(TSParser)* self, uint* length);
 ...
 */
void ts_parser_set_timeout_micros(TSParser* self, ulong timeout);

/**
 * Get the duration in microseconds that parsing is allowed to take.
 */
ulong ts_parser_timeout_micros(const(TSParser)* self);
 ...
 */
uint ts_node_start_byte(TSNode);
 ...
 * child.
 */
TSNode ts_node_child(TSNode, uint);

/**
 * Get the node's number of children.
 */
uint ts_node_child_count(TSNode);
 ...
 */
TSNode ts_node_named_child(TSNode, uint);
 ...
 * See also `ts_node_is_named`.
 */
uint ts_node_named_child_count(TSNode);

/**
 * Get the node's child with the given field name.
 */
TSNode ts_node_child_by_field_name(TSNode self, const(char)* field_name, uint field_name_length);
 ...
 */
TSNode ts_node_first_child_for_byte(TSNode, uint);

/**
 * Get the node's first named child that extends beyond the given byte offset.
 */
TSNode ts_node_first_named_child_for_byte(TSNode, uint);

/**
 * Get the smallest node within this node that spans the given range of bytes
 * or (row, column) positions.
 */
TSNode ts_node_descendant_for_byte_range(TSNode, uint, uint);
 ...
 */
TSNode ts_node_named_descendant_for_byte_range(TSNode, uint, uint);
 ...
 */
long ts_tree_cursor_goto_first_child_for_byte(TSTreeCursor*, uint);
 ...
 */
uint ts_query_pattern_count(const(TSQuery)*);
uint ts_query_capture_count(const(TSQuery)*);
uint ts_query_string_count(const(TSQuery)*);
 ...
 * code strings.
 */
uint ts_query_start_byte_for_pattern(const(TSQuery)*, uint);
 ...
 */
const(char)* ts_query_capture_name_for_id(const(TSQuery)*, uint id, uint* length);
const(char)* ts_query_string_value_for_id(const(TSQuery)*, uint id, uint* length);
 ...
 */
void ts_query_disable_capture(TSQuery*, const(char)*, uint);
 ...
 */
void ts_query_disable_pattern(TSQuery*, uint);
 ...
 * will be executed.
 */
void ts_query_cursor_set_byte_range(TSQueryCursor*, uint, uint);
...
void ts_query_cursor_remove_match(TSQueryCursor*, uint id);
 ...
 */
bool ts_query_cursor_next_capture(TSQueryCursor*, TSQueryMatch* match, uint* capture_index);
 ...
 */
uint ts_language_symbol_count(const(TSLanguage)*);
 ...
 */
uint ts_language_field_count(const(TSLanguage)*);
 ...
 */
TSFieldId ts_language_field_id_for_name(const(TSLanguage)*, const(char)*, uint);
 ...
 */
uint ts_language_version(const(TSLanguage)*);
